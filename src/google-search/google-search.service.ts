...
interface GoogleSearchOrganicResult {
  title?: string;
  link?: string;
  snippet?: string;
  position?: number;
...
@Injectable()
export class GoogleSearchService implements SearchProvider {
  ...
  async search(input: GoogleSearchQueryDto): Promise<SearchResultDto[]> {
    ...
    try {
      const payload = await this.httpClient.getJson<GoogleSearchResponse>(
        requestUrl.toString(),
      );
  ...
  private buildRequestUrl(input: GoogleSearchQueryDto, apiKey: string): URL {
